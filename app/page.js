...
export default function Home() {
  ...
  const onbSteps = [
    ...
    },
    {
      ...
      svg: <svg width="40" height="40" viewBox="0 0 40 40" fill="none">
        ...
        <path d="M8 35h6M18 35h4" stroke="#C3073F" strokeWidth="1.2" strokeLinecap="round" opacity=".4"/>
      </svg>
    },
    {
  ...
  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Syne:wght@400;600;700;800&family=Onest:wght@300;400;500;600;700&display=swap');
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .blob-1 { width: 700px; height: 700px; background: rgba(195,7,63,0.09); top: -250px; left: -200px; }
        .blob-2 { width: 450px; height: 450px; background: rgba(195,7,63,0.06); top: 45vh; right: -180px; animation-delay: -8s; }
        ...
        .nav-login { font-size: 13px; color: rgba(255,255,255,0.35); background: none; border: none; cursor: pointer;
          font-family: 'Onest', sans-serif; padding: 8px 14px; border-radius: 8px; transition: color .2s, background .2s; }
        .nav-login:hover { color: #fff; background: rgba(255,255,255,0.05); }
        .nav-cta { font-size: 13px; font-weight: 600; font-family: 'Onest', sans-serif; color: #fff;
          background: #C3073F; border: none; border-radius: 10px; padding: 9px 20px; cursor: pointer;
        ...
        .hero-tag { display: inline-flex; align-items: center; gap: 8px;
          background: rgba(195,7,63,0.08); border: 1px solid rgba(195,7,63,0.22);
          border-radius: 100px; padding: 6px 16px;
          font-size: 12px; font-weight: 500; color: rgba(255,255,255,0.55); letter-spacing: 0.3px;
        ...
        .hero-h1 { font-family: 'Syne', sans-serif; font-size: clamp(40px,5.8vw,72px); font-weight: 800;
          line-height: 1.06; letter-spacing: -2.5px; max-width: 820px; margin-bottom: 28px;
          animation: fadeUp .7s .1s ease both; }
        .line2 { color: #C3073F; display: block; }
        .line3 { color: rgba(255,255,255,.18); font-weight: 400; display: block; font-style: italic; }
        .hero-sub { font-size: 16px; line-height: 1.8; font-weight: 300; color: rgba(255,255,255,.4);
        ...
        .btn-red { background: #C3073F; color: #fff; border: none; border-radius: 14px; padding: 16px 36px;
          font-size: 15px; font-weight: 600; font-family: 'Onest', sans-serif; cursor: pointer;
          box-shadow: 0 4px 28px rgba(195,7,63,.45); transition: transform .15s, box-shadow .15s; }
        .btn-red:hover { transform: translateY(-2px); box-shadow: 0 8px 44px rgba(195,7,63,.6); }
        .btn-ghost { background: rgba(255,255,255,.045); border: 1px solid rgba(255,255,255,.1);
          color: rgba(255,255,255,.6); border-radius: 14px; padding: 16px 30px;
          font-size: 15px; font-weight: 500; font-family: 'Onest', sans-serif; cursor: pointer;
        ...
        .stat-num { font-family: 'Syne', sans-serif; font-size: 26px; font-weight: 800; display: block; margin-bottom: 4px; }
        ...
        .step:hover { border-color: rgba(195,7,63,.35); background: rgba(195,7,63,.035); transform: translateY(-5px); }
        .step::after { content: ''; position: absolute; bottom: 0; left: 0; right: 0; height: 2px;
          background: linear-gradient(90deg,transparent,#C3073F,transparent); opacity: 0; transition: opacity .3s; }
        .step:hover::after { opacity: .5; }
        .step-num { font-family: 'Syne', sans-serif; font-size: 52px; font-weight: 800;
          color: #C3073F; opacity: .25; line-height: 1; margin-bottom: 22px; }
        ...
        .feat { background: rgba(255,255,255,.02); border: 1px solid rgba(255,255,255,.055);
          ...
          transition: border-color .3s, transform .25s, background .3s; }
        .feat:hover { border-color: rgba(195,7,63,.22); transform: translateY(-3px); background: rgba(195,7,63,.025); }
        .feat-icon { width: 48px; height: 48px; flex-shrink: 0; border-radius: 14px;
          background: rgba(195,7,63,.09); border: 1px solid rgba(195,7,63,.18);
        ...
        .onb-card::before { content: ''; position: absolute; inset: 0;
          background: radial-gradient(ellipse at 50% -10%,rgba(195,7,63,.12),transparent 65%); pointer-events: none; }
        .onb-progress { display: flex; gap: 6px; justify-content: center; margin-bottom: 36px; position: relative; z-index: 1; }
        .onb-pip { height: 4px; border-radius: 2px; transition: all .35s ease; }
        .onb-pip-on { background: #C3073F; width: 32px; }
        .onb-pip-off { background: rgba(255,255,255,.1); width: 16px; }
        .onb-icon-wrap { width: 88px; height: 88px; border-radius: 28px;
        ...
        .cta-h2 { font-family: 'Syne', sans-serif; font-size: 30px; font-weight: 800;
          letter-spacing: -.8px; margin-bottom: 12px; position: relative; z-index: 1; }
        .cta-sub { font-size: 15px; color: rgba(255,255,255,.38); font-weight: 300;
          line-height: 1.7; margin-bottom: 40px; position: relative; z-index: 1; }
        .cta-form { max-width: 360px; margin: 0 auto; position: relative; z-index: 1; }
        .cta-input { width: 100%; background: rgba(255,255,255,.055); border: 1px solid rgba(255,255,255,.09);
          border-radius: 12px; padding: 14px 18px; color: #fff; font-size: 14px;
          font-family: 'Onest', sans-serif; outline: none; margin-bottom: 10px;
        ...
        .cta-submit { width: 100%; background: #C3073F; color: #fff; border: none; border-radius: 12px;
          padding: 16px; font-size: 15px; font-family: 'Onest', sans-serif; font-weight: 700; cursor: pointer;
          box-shadow: 0 4px 28px rgba(195,7,63,.4); transition: transform .15s, box-shadow .15s; margin-top: 4px; }
        .cta-submit:hover { transform: translateY(-2px); box-shadow: 0 8px 44px rgba(195,7,63,.6); }
        .cta-note { font-size: 12px; color: rgba(255,255,255,.18); margin-top: 14px; font-weight: 300; }
        .cta-success { font-family: 'Syne', sans-serif; font-size: 22px; font-weight: 700;
        ...
        .footer-logo em { color: rgba(195,7,63,.45); font-style: normal; }
        .footer-text { font-size: 11px; color: rgba(255,255,255,.16); font-weight: 300; letter-spacing: 0.3px; }
        ...
        @media (max-width: 768px) {
          ...
          .hero-h1 { font-size: clamp(34px,9vw,50px); letter-spacing: -1.8px; }
            ...
            <div className="onb-card">
              ...
              <div className="onb-content" key={onbStep}>
                ...
                <div className="onb-actions">
                  ...
                  {onbStep < onbSteps.length - 1 && (
                    <button className="onb-skip" onClick={() => setOnbStep(onbSteps.length - 1)}>Пропустить</button>
                  )}
                </div>
              </div>
            </div>
          ...
          <div className="cta-box reveal">
            {submitted ? (
              ...
              <>
                ...
                <p className="cta-sub">Оставьте заявку — пришлём QR-код и инструкцию в течение 2 часов. Без предоплаты.</p>
